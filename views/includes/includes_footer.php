<?php
	//this is the footer

?>
</main>  
<footer class="mdl-mini-footer">
  <div class="mdl-mini-footer__left-section">
    <div class="mdl-logo"><button style="color: white;" class="mdl-button mdl-js-button mdl-js-ripple-effect">&copy; : National Institute of technology, Hamirpur</button></div>
  </div>
  
   <div class="mdl-mini-footer__right-section">
   <!-- <div class="mdl-logo">Made with <a style="color: red; font-size: 2em;"><button class="mdl-button mdl-js-button mdl-button icon">
  <i class="material-icons">&hearts;</i>
</button></a> By Team .EXE</div>
  -->
  <div class="mdl-logo"><a href=""><button style="color: white;" class="mdl-button mdl-js-button mdl-js-ripple-effect"> Report bugs</button></a></div>
  <a href=""><button style="color: white;" class="mdl-button mdl-js-button mdl-js-ripple-effect"> Credits</button></a>
  </div>
</footer>

</div>
<script src="../views/design/js/material.min.js"></script>
  </body>
</html>